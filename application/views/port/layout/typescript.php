...
jQuery(document).ready(function(){
  jQuery('#selectLang').change(function(){
    var change = jQuery(this).val();
    if (change == 'kruti_dev') {
      jQuery('#typebox').removeClass().addClass('kruti_dev');
    }else if (change == 'mangal_regular') {
      jQuery('#typebox').removeClass().addClass('mangal');
    }else{
      jQuery('#typebox').removeClass();
    }
  });
});

let sec = parseInt('<?php echo (int)$time * 60; ?>');
...
function addWords() {
  ...
  $("#typebox")[0].value = "";
  for (let i = <?php echo $lenth - 1; ?>; i > 0; i--) {
...
function checkWord(word) {
  ...
  const noMatch = btoa(wval) !== btoa(currentSubstring);
...
function isTimer() {
  ...
  let time = parseInt('<?php echo (int)$time * 60; ?>'); // only set timer once

  let one = $("#timer > span")[0].innerHTML;
  if (one == '<?php echo $time; ?>' ) {
    let typingTimer = setInterval(() => {
      if (time <= 0) {
        ...
        let timePad = fancyTimeFormat(time); // zero padded
...
function calculateWPM(data) {
  ...
  jQuery('#word-section').removeClass();
